...
const GenerateImages = () => {
  ...
  const imageStyle = ['Realistic', 'Ghibli style', 'Anime style', 'Cartoon style',
    ...
    const onSubmitHandler = async(e) => {
      ...
      try {
        setLoading(true)
        const prompt = `Generate an image for the keyword ${input} in the style ${SelectedStyle} 
        `
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <textarea  onChange={(e) =>setInput(e.target.value)} value={input} rows={input} className='w-full p-2 px-3 mt-2 outline-none text-sm
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {imageStyle.map((item) => (
            ...
            cursor-pointer ${SelectedStyle === item.text ? 'bg-green-50 text-green-700' : 
