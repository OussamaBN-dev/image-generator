...
const BlogTitles = () => {
  ...
  const blogCategories = ['General', 'Technologie', 'Business', 'Health', 
  ...
  const onSubmitHandler = async(e) => {
    ...
    try {
      setLoading(true)
      const prompt = `Generate a blog title for the keyword ${input} in cateory ${selectedCategory}`

      const {data} = await axios.post('api/ai/generate-blog-title',{prompt},
        {headers: {Authorization: `Bearer ${await getToken()}`}})
      
        if (data.success){
          setContent(data.cotent)
        } else {
    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        <div className='flex items-center gap-3' >
          ...
          <p className='text-xl font-semibold' >AI Title Generator</p>
        </div>
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {blogCategories.map((item) => (
            ...
            cursor-pointer ${selectedCategory.text === item.text ? 'bg-purple-50 text-purple-700' : 
            'text-gray-500 border-gray-300'}`} key={item} >{item.text}</span>
          ))}
        </div>
        <br />
